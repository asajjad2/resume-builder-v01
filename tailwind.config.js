/** @type {import('tailwindcss').Config} */
module.exports = {
  content: [
    "./app/**/*.{js,ts,jsx,tsx}",
    "./pages/**/*.{js,ts,jsx,tsx}",
    "./components/**/*.{js,ts,jsx,tsx}",
 
    // Or if using `src` directory:
    "./src/**/*.{js,ts,jsx,tsx}",
  ],
  theme: {
    colors: {
      white : '#ffffff',
      black : '#000000',
          '--gray-25':'#fcfcfd',
      gray : {
        25 : '#fcfcfd',
        50 : '#f9fafb',
        100 : '#f2f4f7',
        200 : '#e4e7ec',
        300 : '#d0d5dd',
        400 : '#98a2b3',
        500 : '#667085',
        600 : '#475467',
        700 : '#344054',
        800 : '#1d2939',
        900 : '#101828',
      },
      "blue-gray" : {
        25 : '#fcfcfd',
        50 : '#f8f9fc',
        100 : '#eaecf5',
        200 : '#c8cce5',
        300 : '#9ea5d1',
        400 : '#717bbc',
        500 : '#4e5ba6',
        600 : '#3e4784',
        700 : '#363f72',
        800 : '#293056',
        900 : '#101323',
      },
      "success" : {
        25 : '#f6fef9',
        50 : '#ecfdf3',
        100 : '#d1fadf',
        200 : '#a6f4c5',
        300 : '#6ce9a6',
        400 : '#32d583',
        500 : '#12b76a',
        600 : '#039855',
        700 : '#027a48',
        800 : '#05603a',
        900 : '#054f31',
      },
      "error" : {
        25 : '#fffbfa',
        50 : '#fef3f2',
        100 : '#fee4e2',
        200 : '#fecdca',
        300 : '#fda29b',
        400 : '#f97066',
        500 : '#f04438',
        600 : '#d92d20',
        700 : '#b42318',
        800 : '#912018',
        900 : '#7a271a',
      },
      "warning" : {
        25 : '#fffcf5', 
        50 : '#fffaeb',
        100 : '#fef0c7',
        200 : '#fedf89',
        300 : '#fec84b',
        400 : '#fdb022',
        500 : '#f79009',
        600 : '#dc6803',
        700 : '#b54708',
        800 : '#93370d',
        900 : '#7a2e0e',
      },
      orange : {
        25 : '#fffafd',
        50 : '#fff6ed',
        100 : '#ffead5',
        200 : '#fddcab',
        300 : '#feb273',
        400 : '#fd853a',
        500 : '#fb6514',
        600 : '#ec4a0a',
        700 : '#c4320a',
        800 : '#9c2a10',
        900 : '#7e2410',
      },
      "blue-light" : {
        25 : '#f5fbff',
        50 : '#f0f9ff',
        100 : '#e0f2fe',
        200 : '#b9e6fe',
        300 : '#7cd4fd',
        400 : '#36bffa',
        500 : '#0ba5ec',
        600 : '#0086c9',
        700 : '#026aa2',
        800 : '#065986',
        900 : '#0b4a6f',
      },
      blue: {
        25 : '#f5faff',
          
        50 : '#eff8ff',
        100 : '#d1e9ff',
        200 : '#b2ddff',
        300 : '#84caff',
        400 : '#53b1fd',
        500 : '#2e90fa',
        600 : '#1570ef',
        700 : '#175cd3',
        800 : '#1849a9',
        900 : '#194185',
      },
      indigo : {
        25 : '#f5f8ff',
        50 : '#eef4ff',
        100 : '#e0eaff',
        200 : '#c7d7fe',
        300 : '#a4bcfd',
        400 : '#8098f9',
        500 : '#6172f3',
        600 : '#444ce7',
        700 : '#3538cd',
        800 : '#2d31a6',
        900 : '#2d3282',
      },
      purple : {
        25 : '#fcfaff',
        50 : '#f9f5ff',
        100 : '#f4ebff',
        200 : '#e9d7fe',
        300 : '#d6bbfb',
        400 : '#b692f6',
        500 : '#9e77ed',
        600 : '#7f56d9',
        700 : '#6941c6',
        800 : '#53389e',
        900 : '#42307d',
      },
      pink : {
        25 : '#fef6fb',
        50 : '#fdf2fa',
        100 : '#fce7f6',
        200 : '#fcceee',
        300 : '#faa7e0',
        400 : '#f670c7',
        500 : '#ee46bc',
        600 : '#dd2590',
        700 : '#c11574',
        800 : '#9e165f',
        900 : '#851651',
      },
      rose : {
        25 : '#fff5f6',
        50 : '#fff1f3',
        100 : '#ffe4e8',
        200 : '#fecdd6',
        300 : '#fea3b4',
        400 : '#fd6f8e',
        500 : '#f63d68',
        600 : '#e31b54',
        700 : '#c01048',
        800 : '#a11043',
        900 : '#89123e',
      },
    },

    fontWeight : {
      extrabold : '800',
      bold : '700',
      semibold : '600',
      medium : '500',
      normal : '400',
    },

    fontSize : {
      "text-xl" : ['1.25rem', '1.875rem'],
      "text-xl2" : ['1.5rem', '2rem'],
      "text-lg" : ['1.125rem', '1.625rem'],
      "text-md" : ['1rem','1.5rem'],
      "text-sm" : ['0.875rem', '1.25rem'],
      "text-xs" : ['0.75rem', '1.125rem'],
      "caps-heading" : ['1rem', '1.188rem'],
      "display-2xl" : ['4.5rem', '5.625rem'],
      "display-xl" : ['3.75rem', '4.5rem'],
      "display-lg" : ['3rem', '3.75rem'],
      "display-md" : ['2.25rem', '2.75rem'],
    },

    extend: {
      boxShadow : {
        lg : '0px 4px 6px -2px rgba(16,24,40,0.05), 0px 12px 16px -4px rgba(16,24,40,0.1)px',
        xl : '0px 8px 8px -4px rgba(16,24,40,0.04), 0px 20px 24px -4px rgba(16,24,40,0.1)px'
      },

      fontFamily : {
        inter : ['Inter', 'sans-serif'],
        pd : ['Playfair Display', 'serif'],
      },

    },
  },
  plugins: [],
}
